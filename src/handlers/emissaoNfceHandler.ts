...
import { NFCeDataDto, CertificadoConfigDto, SefazResponseDto } from '../dto';
import {
...
export class EmissaoNfceHandler {
  ...
  async emitirNFCe(
    ...
    try {
      const xmlNFCe = this.criarXMLNFCe(dados);

      this.salvarArquivoDebug(xmlNFCe, 'nfce_original');

      const xmlAssinado = await tools.xmlSign(xmlNFCe);

      this.salvarArquivoDebug(xmlAssinado, 'nfce_assinado');
      ...
      this.salvarArquivoDebug(xmlResponse, 'sefaz_resposta');
  ...
  private criarXMLNFCe(dados: NFCeDataDto): string {
    const NFe = new Make();
    ...
    dados.produtos.forEach((_, index) => {
      ...
      NFe.tagProdPIS(index, {
        CST: impostos.CST_PIS,
        qBCProd: '0.0000',
        vAliqProd: '0.0000',
        vPIS: '0.00',
      });
      NFe.tagProdCOFINS(index, {
        CST: impostos.CST_COFINS,
        qBCProd: '0.0000',
        vAliqProd: '0.0000',
        vCOFINS: '0.00',
      });
  ...
  private processarResposta(xmlResposta: string): SefazResponseDto {
  ...
  private async fazerRequisicaoSefaz(
    url: string,
    xmlDados: string,
    certificadoConfig: CertificadoConfigDto,
  ): Promise<string> {
