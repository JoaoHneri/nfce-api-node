...
import { CacheStatsDto, CertificadoConfigDto } from '../dto';
import { CacheEntry, CacheConfig } from '../types/cacheTypes';
...
export class ToolsCache {
  ...
  async obterTools(certificadoConfig: CertificadoConfigDto): Promise<Tools> {
    ...
    if (
      cached &&
      CacheUtils.isEntradaValida(cached.timestamp, this.config.ttl)
    ) {
      // Cache HIT
      cached.hits++;
      ...
      const tools = new Tools(
        {
          ...
          openssl:
            'C:/Users/joaoh/Downloads/windowsLibs/libs/openssl-3.5.0.win86/bin/openssl.exe' as any,
  ...
  private controlarTamanhoCache(): void {
    if (this.cache.size > this.config.maxSize) {
      ...
      const entradas = Array.from(this.cache.entries()).sort(([, a], [, b]) => {
  ...
  public obterEstatisticas(): CacheStatsDto {
    const agora = Date.now();

    const empresas = Array.from(this.cache.entries()).map(
      ([chave, cached]) => ({
        empresa: CacheUtils.mascararEmpresa(chave),
        hits: cached.hits,
        idade: CacheUtils.formatarTempo(agora - cached.timestamp),
        expiraEm: CacheUtils.formatarTempo(
          CacheUtils.tempoParaExpirar(cached.timestamp, this.config.ttl),
        ),
      }),
    );
  ...
  public removerEmpresa(certificadoConfig: CertificadoConfigDto): boolean {
